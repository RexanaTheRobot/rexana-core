...
#client_socket.connect(('192.168.178.21', 8000))
...
try:
    ...
    start = time.time()
    finish = time.time()
    seconds = 10 # how many seconds to stream frames for
    ...
    with picamera.PiCamera() as camera:
        ...
        time.sleep(2)
        start = time.time()
        camera.capture_sequence(streams(seconds), 'jpeg', use_video_port=True)
